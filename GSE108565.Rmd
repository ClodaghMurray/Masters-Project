...
```{r}
library(pheatmap)
pheatmap(sig_exprs, trace="none", scale="row", cexRow=0.2, cexCol=0.7, ColSideColors = orientation, show_rownames = F, cluster_cols = T,)
```
...
dim(datExpr) # top 70% most variable genes of data
...
```{r}
mod.paleturquoise <- names(datExpr)[moduleColors=="paleturquoise"] # pos 265 genes
...
dotplot(ego, showCategory=10) + ggtitle("PaleTurqoise Module GO (Biological Process)")  +
  theme_classic()
#dev.off()
```
...
venn.diagram(x = list(DEGs$ENTREZID, mod.paleturquoise_entrez$ENTREZID),
           ...
           filename = '/home/clodagh/MASTERS PROJECT/GSE37614/GSE108565_DGE_Module_Overlap.png',
             output = T,
             imagetype = 'png',
             scaled = F,
             col = 'grey70',
...
library('png')
pp <- readPNG("/home/clodagh/MASTERS PROJECT/GSE37614/GSE108565_DGE_Module_Overlap.png")
plot.new()
...
Background <- nrow(probes) #all genes used in the wgcna analysis (~8.5k genes)
...
pval.modpaleturquoise # 2.558051e-217 VERY significantly enriched for up regulated genes

#FF is pink, #40E is turquoise
#colours <- c('#CE2F17', '#E0FFFF', '#88D969', '#FFC0CB', '#40E0D0')
colours <- c('#40E0D0', '#CE2F17')
venn.diagram(x = list(up_reg$ENTREZID, mod.paleturquoise_entrez$ENTREZID),
             category.names = c('Paleturqoise Module', 'Upregulated DEG\'s'),
             filename = '/home/clodagh/MASTERS PROJECT/GSE37614/UpregPaleturqoise_Module_Overlap.png',
             output = T,
             main = '',
...
options(repr.plot.height=12, repr.plot.width = 12)
library('png')
pp <- readPNG('/home/clodagh/MASTERS PROJECT/GSE37614/UpregPaleturqoise_Module_Overlap.png')
plot.new()
rasterImage(pp, 0,0,1.1, 1.1)
