...
datExpr <- varFilter(datExpr, var.cutoff = 0.7) #filter threshold top 30% of most variable genes 
dim(datExpr) 
...
table(dynamicMods) # 27 modules detected
collectGarbage()
```
...
```{r}
mod.darkorange <- names(datExpr)[moduleColors=="darkorange"] # 176 genes
names(mod.darkorange) <- rownames(mod.darkorange)
mod.darkorange_entrez <- AnnotationDbi::select(hs, 
       keys = mod.darkorange,
       columns = c("ENTREZID", "SYMBOL"),
       keytype = "SYMBOL")
#remove NAs
mod.darkorange_entrez <- mod.darkorange_entrez[rowSums(is.na(mod.darkorange_entrez)) != ncol(mod.darkorange_entrez), ]
```

```{r}
#265 genes all up regu;ated
list <- tT[mod.darkorange,]$adj.P.Val
names(list) <- rownames(tT[mod.darkorange,])
...
ego <- enrichGO(gene = mod.darkorange_entrez$ENTREZID, 
...
pdf(file='/home/clodagh/MASTERS PROJECT/GSE37614/GSE108565_darkorange_ORA_BP.pdf')
dotplot(ego, showCategory=10) + ggtitle("Darkorange Module GO (Biological Process)")  +
  theme_classic()
#dev.off()
```
...
cluster_BP_summary <- data.frame(ego)
View(cluster_BP_summary)
pdf(file='/home/clodagh/MASTERS PROJECT/GSE37614/GSE108565_darkorange_CC.pdf')
...
cluster_BP_summary <- data.frame(ego)
#View(cluster_BP_summary)
pdf(file='/home/clodagh/MASTERS PROJECT/GSE37614/GSE108565_darkorange_ORA_MF.pdf')
...
ekegg <- enrichKEGG(gene = mod.darkorange_entrez$ENTREZID, 
                organism = "hsa",
                keyType = 'kegg',
           #     keyType = "ENTREZID",
                qvalueCutoff = 0.05, 
...
dotplot(ekegg, showCategory=10, font.size = 8) + scale_y_discrete(labels = function(x) str_wrap(x, width = 20)) +
  ggtitle("darkorange Module GO (Molecular Function)")  + 
  theme_classic() 
#dev.off()
```

Overlap between DEGs and darkorange module
```{r}
...
library(VennDiagram)
colours <- c('#E0FFFF', "#ff8c00")
venn.diagram(x = list(DEGs$ENTREZID, mod.darkorange_entrez$ENTREZID),
             category.names = c('DEG\'s', 'Darkorange Module'),
           filename = '/home/clodagh/MASTERS PROJECT/GSE37614/GSE108565_DGE_darkorange_Module_Overlap.png',
             output = T,
             imagetype = 'png',
             scaled = F,
             col = 'grey70',
...
library('png')
pp <- readPNG("/home/clodagh/MASTERS PROJECT/GSE37614/GSE108565_DGE_darkorange_Module_Overlap.png")
plot.new()
...
#upreg
#182 upregulated genes in the darkorange module
length(intersect(up_reg$ENTREZID, mod.darkorange_entrez$ENTREZID))
# 0 downregulated genes in the darkorange module
length(intersect(down_reg$ENTREZID, mod.darkorange_entrez$ENTREZID))
#downreg
```

```{r}
#test for the statistical significance of these overlaps using hypergeometric test
Overlap <- length(intersect(up_reg$ENTREZID, mod.darkorange_entrez$ENTREZID))
Group1 <- length(up_reg$ENTREZID)
Group2 <- length(mod.darkorange_entrez$ENTREZID)
Background <- length(probes) #all genes used in the wgcna analysis (~8.5k genes)
pval.moddarkorange <- phyper(Overlap,Group2, Background - Group2, Group1, lower.tail = F )  #if less than 0.05 assume significant enrichment
pval.moddarkorange # 2.188301e-285 VERY significantly enriched for up regulated genes
#FF is pink, #40E is turquoise
#colours <- c('#CE2F17', '#E0FFFF', '#88D969', '#FFC0CB', '#40E0D0')
colours <- c('#E0FFFF', "#ff8c00")
venn.diagram(x = list(up_reg$ENTREZID, mod.darkorange_entrez$ENTREZID),
             category.names = c('Dark Orange Module', 'Upregulated DEG\'s'),
             filename = '/home/clodagh/MASTERS PROJECT/GSE37614/UpregDarkOrange_Module_Overlap.png',
             output = T,
             main = '',
...
options(repr.plot.height=12, repr.plot.width = 12)
library('png')
pp <- readPNG('/home/clodagh/MASTERS PROJECT/GSE37614/UpregDarkOrange_Module_Overlap.png')
plot.new()
rasterImage(pp, 0,0,1.1, 1.1)
```

```{r}
#use these to filter the modules filter darkorange module for export to cytoscape
filter_darkorange <- intersect(up_reg$SYMBOL, mod.darkorange_entrez$SYMBOL)
```
...
library(RCy3)
cytoscapePing()
edge <- read.delim("/home/clodagh/MASTERS PROJECT/GSE37614/CytoscapeInput-edges-darkorange.txt")
colnames(edge) <- c("source", "target", "weight", "direction", "fromAltName", "toAltName")
node <-  read.delim("/home/clodagh/MASTERS PROJECT/GSE37614/CytoscapeInput-nodes-darkorange.txt")
colnames(node) <- c("id", "altName", "node_attributes")
createNetworkFromDataFrames(node, edge, title = "First 50")
```

#Importing darkorange module
...
write.table(cyt$edgeData, file = "/home/clodagh/MASTERS PROJECT/GSE37614/GSE108565_cyt_darkorangeedge_noMMGS_noTOMfilter.txt", sep = "\t ", row.names = F, col.names = TRUE, quote = F)
write.table(cyt$nodeData, file = "/home/clodagh/MASTERS PROJECT/GSE37614/GSE108565_cyt_darkorangenode_noMMGS_noTOMfilter.txt", sep = "\t ", row.names = F, col.names = TRUE, quote = F)
```
