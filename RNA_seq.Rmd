...
```{r}
library(WGCNA)
counts_matrix <- as.data.frame(t(rld))
# filter genes with lots of zero counts
gsg = goodSamplesGenes(counts_matrix, verbose = 3)
datExpr0 = counts_matrix[gsg$goodSamples, gsg$goodGenes]
datExpr0
```
...
clust = cutreeStatic(sampleTree, cutHeight = 50, minSize = 5)
table(clust)
...
traitRows = match(samps, samples$ID);
datTraits = samples[traitRows, ];
dim(datTraits)
rownames(datTraits) = samples[traitRows, 10];
datTraits
collectGarbage();
```

```{r}
save(datExpr, datTraits, file = "CAFsdataInput.RData")
```


## Network Construction and Module Detection
```{r}
lnames = load(file = "CAFsdataInput.RData");
# The variable lnames contains the names of loaded variables.
lnames
# Get the number of sets in the multiExpr structure.
nSets = checkSets(datExpr, checkStructure = T)$nSets
...
for (set in 1:nSets)
  powerTables[[set]] = list(data = pickSoftThreshold(datExpr[[set]], powerVector=powers,
                                                     verbose = 2)[[2]]);
collectGarbage();
# Plot the results:
colors = c("black", "red")
