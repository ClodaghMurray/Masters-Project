...
rm_batch <- as.matrix(rm_batch[,-23])
...
table(rownames(datTraits) == rownames(norm_counts))   
```
...
sft = pickSoftThreshold(norm_counts, powerVector=powers, networkType = "signed", verbose =0) #call network topology analysis function
```
...
# For computational reasons, restrict to 5000 most connected genes
```{r}
datExpr=norm_counts[, rank(-k,ties.method="first" )<=5000]
```
...
heatmap.2(t(adj[heatmap_indices, heatmap_indices]),
            ...
            xlab='Gene', ylab='Gene',
            main='     Adjacency matrix (pearson, power:22)',
...
table(dynamicMods) #44 modules detected
collectGarbage()
...
colorOfColumn
par(mfrow = c(2, 2))
selectModules = c("darkturquoise", "darkgreen", "darkmagenta")
...
rm_colorlevels = c("salmon","red","saddlebrown", "brown", "turquoise", "black", "magenta", "lightgreen", "midnightblue", "green" )
...
names(datExpr)[moduleColors=="darkturquoise"] 
