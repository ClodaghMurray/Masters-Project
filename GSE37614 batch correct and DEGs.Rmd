...
write.table(sig_exprs, "GSE37614_DEGs_overall.csv", row.names=T, sep="\t")
...
```{r}
#Top Table of significant genes only
up_reg_fc2 <- top1[top1$adj.P.Val <= 0.05 & abs(top1$logFC) >= 2, ];
nrow(up_reg_fc2)
```
### (Her2-TNBC)
```{r}
...
### Run pheatmap
pheatmap(sig_exprs_, color = heat.colors, cluster_rows = F, show_rownames=T,
annotation= annot_col, border_color=NA, fontsize = 10, scale="row",
     fontsize_row = 10, height=20, main = "Top 40 DEGs")
```
