...
#dissTOM <- TOMdist(adj)
dissTOM <- read.csv('/home/clodagh/MASTERS PROJECT/GSE37614/GSE37614_dissTOM.txt')
#save TOM 
...
table(dynamicMods) # 18 modules detected
collectGarbage()
...
colorOfColumn
par(mfrow = c(2, 2))
selectModules = c("lightgreen", "midnightblue")
...
names(datExpr)[moduleColors== "mignightblue"] # 257 genes pos association
...
```{r}
# modules of interest for HER2
mod.lightgreen <- names(datExpr)[moduleColors=="lightgreen"] # 442 genes negative association
mod.midnightblue<- names(datExpr)[moduleColors== "midnightblue"]# 394 genes pos association
...
mod.midnightblue_entrez <- AnnotationDbi::select(hs, 
       keys = mod.midnightblue,
       columns = c("ENTREZID", "SYMBOL"),
       keytype = "SYMBOL")
```
...
ego <- enrichGO(gene = mod.midnightblue_entrez$ENTREZID, 
                universe = probes2annot$ENTREZID,
                keyType = "ENTREZID",
                OrgDb = org.Hs.eg.db, 
                ont = "BP", #Biological Processes GO term, also done for CC and MF
...
cluster_BP_summary <- data.frame(ego)
View(cluster_BP_summary)
pdf(file='/home/clodagh/MASTERS PROJECT/GSE37614/Enrichment_Analysis/midnightblue_ORA_BP.pdf')
dotplot(ego, showCategory=10, font.size = 8) + scale_y_discrete(labels = function(x) str_wrap(x, width = 20)) + ggtitle("Midnightblue Module GO (Biological Process)")  + 
  theme_classic() 
dev.off()
ego <- enrichGO(gene = mod.midnightblue_entrez$ENTREZID, 
                universe = probes2annot$ENTREZID,
...
View(cluster_CC_summary)
#can see a llot of terms associated with chromosomal rearrangement
pdf(file='/home/clodagh/MASTERS PROJECT/GSE37614/Enrichment_Analysis/midnightblue_ORA_CC.pdf')
dotplot(ego, showCategory=10, font.size = 8) + scale_y_discrete(labels = function(x) str_wrap(x, width = 20)) + ggtitle("Midnightblue Module GO (Cellular Compartment)")  + 
  theme_classic() 
dev.off()
ego <- enrichGO(gene = mod.midnightblue_entrez$ENTREZID, 
                universe = probes2annot$ENTREZID,
...
cluster_MF_summary <- data.frame(ego)
View(cluster_MF_summary)
pdf(file='/home/clodagh/MASTERS PROJECT/GSE37614/Enrichment_Analysis/midnightblue_ORA_MF.pdf')
dotplot(ego, showCategory=10, font.size = 8) + scale_y_discrete(labels = function(x) str_wrap(x, width = 20)) +
  ggtitle("Midnightblue Module GO (Molecular Function)")  + 
  theme_classic() 
dev.off()
#KEGG Pathway Enrichment Analysis
ekegg <- enrichKEGG(gene = mod.midnightblue_entrez$ENTREZID, 
                organism = "hsa",
                keyType = 'kegg',
           #     keyType = "ENTREZID",
                qvalueCutoff = 0.05, 
...
dotplot(ekegg, showCategory=10) + ggtitle("Midnightblue Module GO (Molecular Function)")  +
  theme_classic()
dev.off()
```
...
```{r}
#list of gene names 
de_gene <- fc05$ENTREZID
de_gene <- de_gene[-78]
# now in same format as the module genes. look at intersection

#remove NAs
mod.lightgreen_entrez <- mod.lightgreen_entrez[c(-72,-275),]
mod.midnightblue_entrez
#Use the Venn Diagram package in R
library(VennDiagram)
colours <- c('#CE2F17', '#88D969', 'blue')
venn.diagram(x = list(de_gene, mod.lightgreen_entrez$ENTREZID, mod.midnightblue_entrez$ENTREZID),
             category.names = c('DEG\'s', 'lightgreen Module', 'midnightblue Module'),
...
#using rownames 
toptable <- read.csv("/home/clodagh/MASTERS PROJECT/GSE37614/toptable.csv", row.names = 1)
up_reg <- toptable[toptable$adj.P.Val <= 0.05 & (toptable[,1] > 1) | (toptable[,2] > 1), ];
nrow(up_reg)  

down_reg <- toptable[toptable$adj.P.Val <= 0.05 & (toptable[,1] < -1) | (toptable[,2] < -1), ];
nrow(down_reg)
...
#upreg
# 134 upregulated genes in the darkgreen module
length(intersect(up_reg_entrez$ENTREZID, mod.lightgreen_entrez$ENTREZID))

# 0 upregulated genes in the lightgreen modules
length(intersect(up_reg_entrez$ENTREZID, mod.midnightblue_entrez$ENTREZID))
...
length(intersect(down_reg_entrez$ENTREZID, mod.midnightblue_entrez$ENTREZID))
...
colours <- c('#FFC0CB','#88D969')
venn.diagram(x = list(up_reg_entrez$ENTREZID, mod.lightgreen_entrez$ENTREZID),
             category.names = c('Upregulated DEG\'s','lightgreen Module'),
             filename = '/home/clodagh/MASTERS PROJECT/GSE37614/Enrichment_Analysis/Upreglightgreen_Module_Overlap.png',
             output = T,
             main = '',
             sub = 'Overlap of DEG\'s and Lightgreen Module',
             imagetype = 'png',
             scaled = F,
             col = 'grey70',
             alpha = rep(0.5, 2),
...
options(repr.plot.height=12, repr.plot.width = 12)
library('png')
pp <- readPNG('/home/clodagh/MASTERS PROJECT/GSE37614/Enrichment_Analysis/Upreglightgreen_Module_Overlap.png')
plot.new()
rasterImage(pp, 0,0,1.1, 1.1)
```
...
# Select modules
inModule= is.finite(match(annot$SYMBOL, mod.lightgreen_entrez$SYMBOL)) #so this now filters the module by intersect
#inModule
...
cyt_lightgreen = exportNetworkToCytoscape(modTOM,
...
```
```{r}
modules = "mignightblue"
inModule= is.finite(match(annot$SYMBOL, mod.midnightblue_entrez$SYMBOL)) #so this now filters the module by intersect
#inModule
#inModule = is.finite(match(moduleColors, modules));
modProbes = annot[inModule,]$SYMBOL;
modGenes = annot[inModule,]$ENTREZID;
...
dimnames(modTOM) = list(modGenes, modGenes)
# Export the network into edge and node list files Cytoscape can read
cyt_midnightblue = exportNetworkToCytoscape(modTOM,
edgeFile = paste("GSE37614_CytoscapeInput-edges-", paste(modules, collapse="-"), ".txt", sep=""),
nodeFile = paste("GSE37614_CytoscapeInput-nodes-", paste(modules, collapse="-"), ".txt", sep=""),
threshold = 0.1,
nodeNames = modProbes,
