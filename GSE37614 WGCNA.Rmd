...
rm_batch <- read.csv("/home/clodagh/MASTERS PROJECT/GSE37614/GSE37614_rm_batch_.txt", header = T, sep= "\t")
...
sft = pickSoftThreshold(datExpr, powerVector=powers, dataIsExpr = T, networkType = "signed", verbose =2) #call network topology analysis function
```
...
# Plot the result
MEDissThres = 0.30 #correlation of 75% to merge. 
sizeGrWindow(7, 6)
...
colorOfColumn
par(mfrow = c(2, 2))
selectModules = c("floralwhite", "darkgreen")
...
rm_colorlevels = c("orangered4","cyan", "lightyellow", "turquoise", "blue", "brown", "yellow", "green")
...
names(datExpr)[moduleColors=="floralwhite"] # 793 genes pos association
names(datExpr)[moduleColors== "darkgreen"] # 532 genes neg association
...
```{r}
# modules of interest for HER2
mod.floralwhite <- names(datExpr)[moduleColors=="floralwhite"] # 278 genes negative association
mod.darkgreen<- names(datExpr)[moduleColors== "darkgreen"]# 631 genes pos association
#mod.darkgreen<- names(datExpr)[moduleColors== "darkgreen"] # 608 genes pos association
```


```{r}
mod.floralwhite_entrez <- AnnotationDbi::select(hs, 
       keys = mod.floralwhite,
       columns = c("ENTREZID", "SYMBOL"),
       keytype = "SYMBOL")
...
#try floralwhite first as neg enriched in HER2 status
ego <- enrichGO(gene = mod.floralwhite_entrez$ENTREZID, 
...
pdf(file='/home/clodagh/MASTERS PROJECT/GSE37614/Enrichment_Analysis/floralwhite_ORA_BP.pdf')
dotplot(ego, showCategory=10) + ggtitle("floralwhite Module GO (Biological Process)")  +
  theme_classic()
dev.off()
...
dotplot(ego, showCategory=10) + ggtitle("floralwhite Module GO (Cellular Component)")  +
  theme_classic()
dev.off()
ego <- enrichGO(gene = mod.floralwhite_entrez$ENTREZID, 
                universe = probes2annot$ENTREZID,
...
pdf(file='/Users/martingordon/Documents/MSC_project/data/new_WGCNA/Enrichment_Analysis/floralwhite_ORA_MP.pdf')
dotplot(ego, showCategory=10) + ggtitle("floralwhite Module GO (Molecular Function)")  +
  theme_classic()
dev.off()

#KEGG Pathway Enrichment Analysis
ekegg <- enrichKEGG(gene = mod.floralwhite_entrez$ENTREZID, 
                organism = "hsa",
                keyType = 'kegg',
                #keyType = "ENTREZID",
                qvalueCutoff = 0.05, 
...
#pdf(file='/Users/martingordon/Documents/MSC_project/data/new_WGCNA/Enrichment_Analysis/floralwhite_ORA_KEGG.pdf')
dotplot(ekegg, showCategory=10) + ggtitle("floralwhite Module GO (Molecular Function)")  +
  theme_classic()
dev.off()
```
...
dotplot(ego, showCategory=10, font.size = 8) + scale_y_discrete(labels = function(x) str_wrap(x, width = 20)) + ggtitle("darkgreen Module GO (Biological Process)")  + 
  theme_classic() 
dev.off()
ego <- enrichGO(gene = mod.darkgreen_entrez$ENTREZID, 
                universe = probes2annot$ENTREZID,
...
#can see a llot of terms associated with chromosomal rearrangement
pdf(file='/home/clodagh/MASTERS PROJECT/GSE37614/Enrichment_Analysis/darkgreen_ORA_CC.pdf')
dotplot(ego, showCategory=10, font.size = 8) + scale_y_discrete(labels = function(x) str_wrap(x, width = 20)) + ggtitle("darkgreen Module GO (Cellular Compartment)")  + 
  theme_classic() 
dev.off()
ego <- enrichGO(gene = mod.darkgreen_entrez$ENTREZID, 
                universe = probes2annot$ENTREZID,
...
#Can see from plot that many of the differentially expressed genes are located in the floralwhite and turquoise modules, as floralwhite module has ~ 750 genes and locates ~250 of the DGE's seems like an interesting module to investigate.. Over 1/3 of these modules genes are located in the DGE list

```{r}
#list of gene names 
de_gene <- DEG_entrez$ENTREZID
# now in same format as the module genes. look at intersection
mod.darkgreen_entrez <- mod.darkgreen_entrez[-92,]
#Use the Venn Diagram package in R
library(VennDiagram)
colours <- c('#CE2F17', '#88D969', 'blue')
venn.diagram(x = list(de_gene, mod.floralwhite_entrez$ENTREZID, mod.darkgreen_entrez$ENTREZID),
             category.names = c('DEG\'s', 'floralwhite Module', 'darkgreen Module'),
...
TOM = TOMsimilarityFromExpr(datExpr, power = 24 , corType = "bicor", networkType = "signed");
head(TOM[1:5,1:5])
```
...
# Select modules
inModule= is.finite(match(annot$SYMBOL, mod.floralwhite)) #so this now filters the module by intersect
#inModule
...
cyt_floralwhite = exportNetworkToCytoscape(modTOM,
...
```{r}
modules = "darkgreen"
inModule= is.finite(match(annot$SYMBOL, mod.darkgreen)) #so this now filters the module by intersect
#inModule
#inModule = is.finite(match(moduleColors, modules));
modProbes = annot[inModule,]$SYMBOL;
modGenes = annot[inModule,]$ENTREZID;
...
dimnames(modTOM) = list(modGenes, modGenes)
# Export the network into edge and node list files Cytoscape can read
cyt_darkgreen = exportNetworkToCytoscape(modTOM,
edgeFile = paste("GSE37614_CytoscapeInput-edges-", paste(modules, collapse="-"), ".txt", sep=""),
nodeFile = paste("GSE37614_CytoscapeInput-nodes-", paste(modules, collapse="-"), ".txt", sep=""),
threshold = 0.1,
nodeNames = modProbes,
